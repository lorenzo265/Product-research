...
Buscas web: 15 (leituras de página via `curl`/`r.jina.ai` não contam no orçamento)
...
- **f-2026-0176 (IN RFB 2.292/2025) falhou na conferência automática de trecho** (`python3 -m harness conferir-trecho`): jusbrasil.com.br devolve HTTP 403 tanto ao `curl` direto quanto a tentativas com outro user-agent, e o leitor alternativo do harness também não confirmou o trecho. Reli a página duas vezes via `r.jina.ai` (D-015, página pública sem login), com conteúdo idêntico nas duas leituras e igual ao texto citado no fato. Registro como lacuna de acesso do verificador automático a essa fonte específica, não como erro de citação — mas o juiz deve saber que essa fonte não passou pela conferência mecânica.
- planalto.gov.br (Decreto 6.759/2009) devolveu HTTP 503 ao `curl` direto nesta sessão; o fato f-2026-0174 foi lido via `r.jina.ai` e passou na conferência automática normalmente.
